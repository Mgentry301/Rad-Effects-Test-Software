...
import numpy as np
import argparse
from PythonScripts.AD.AD9082.API.ad9082_python_eval_app import settings
...
def nco_test(use_case, n_loops=10, interactive=False, dev_config=None):
...
def analog_loopback_test(use_case, do_init=True, n_loops=10, interactive=False, txfe_dpg=None, dev_config=None, ad9082=None):
    ...
    # Display the jesd link status
    print_link_status(use_case, ad9082)
...
def real_data_test(use_case, txfe_dpg=None, dev_config=None):
...
def print_link_status(use_case, ad9082):
...
def init(uc_arg=None, realonly=None):
    import argparse


    parser = argparse.ArgumentParser()
    parser.add_argument("--uc", help="use case number. See associated use_case_xx.py file.", type=int, default=3)
    parser.add_argument("--realonly", help="real data only", type=int, required=False, default=0)
    args, _ = parser.parse_known_args([])  # Don't parse sys.argv when called as a function

    if uc_arg is not None:
        args.uc = uc_arg
    if realonly is not None:
        args.realonly = realonly
    ...
    analog_loopback_test(uc, True, n_loops=5, interactive=True, txfe_dpg=txfe_dpg, dev_config=dev_config, ad9082=ad9082) 


    return ads9, ad9082, uc, args

if __name__ == '__main__':
    ads9, ad9082, uc, args = init()  
